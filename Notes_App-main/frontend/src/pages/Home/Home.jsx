...
const Home = () => {
  ...
  // Mock user info (replace backend fetch)
  const getUserInfo = () => {
  ...
  // Mock notes (replace backend fetch)
  const getAllNotes = () => {
    // Example static notes
    setAllNotes([
      { _id: '1', title: 'First Note', content: 'This is the first note', createdAt: new Date(), isPinned: false },
      { _id: '2', title: 'Second Note', content: 'This is the second note', createdAt: new Date(), isPinned: true },
    ]);
  };

  const deleteNote = (data) => {
    setAllNotes(allNotes.filter((note) => note._id !== data._id));
  };
